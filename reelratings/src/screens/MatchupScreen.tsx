...
export default function MatchupScreen() {
  const { focusMovieId: focusParam } = useLocalSearchParams<{ focusMovieId?: string }>()
  ...
  useEffect(() => { focusMovieRef.current = focusMovie }, [focusMovie])
  ...
  const seedMoviesIfNeeded = async (uid: string) => {
    ...
    try {
      ...
      const [topRatedPages, popularPages] = await Promise.all([
        Promise.all(pages.map(p => getTopRatedMovies(p))),
        Promise.all(pages.map(p => getPopularMovies(p))),
      ])

      const allMovies = [...topRatedPages.flat(), ...popularPages.flat()]
      ...
      const chunkSize = 50
      for (let i = 0; i < movieRows.length; i += chunkSize) {
        await supabase
          .from('movies')
          .upsert(movieRows.slice(i, i + chunkSize), { onConflict: 'id' })
      }

      const userMovieRows = unique.map(m => ({
        user_id: uid,
      ...
      for (let i = 0; i < userMovieRows.length; i += chunkSize) {
        await supabase
          .from('user_movies')
          .upsert(userMovieRows.slice(i, i + chunkSize), { onConflict: 'user_id,movie_id' })
      }
    } finally {
      setSeeding(false)
    }
  ...
  const loadNextPair = async (uid: string, overrideFocusMovie?: Movie | null) => {
    setLoading(true)
    setSwipeHint(null)
    try {
      // Use override if provided (handles the case where state hasn't updated yet)
      const fm = overrideFocusMovie !== undefined ? overrideFocusMovie : focusMovieRef.current
      ...
      if (fm) {
        ...
        if (!focusData) {
          ...
          const opponent = rowToMovie(opponents[Math.floor(Math.random() * Math.min(opponents.length, 5))])
          const pair: [Movie, Movie] = Math.random() > 0.5
            ? [updatedFm, opponent]
            : [opponent, updatedFm]
      ...
      const pair: [Movie, Movie] = [rowToMovie(shuffled[0]), rowToMovie(shuffled[1])]
      setMovies(pair)
      moviesRef.current = pair
      resetAnimations()
    } finally {
      ...
      onPanResponderRelease: (_, gs) => {
        ...
        if (dir === 'vertical') {
          ...
          Animated.parallel([
            ...
            resetAnimations()
            if (dy < 0) handleVote(current[0].id, current[1].id)  // swipe up → top wins
    ...
    <View style={styles.wrapper} {...panResponder.panHandlers}>
      ...
      {focusMovie && (
        <View style={styles.focusBanner}>
          ...
          <TouchableOpacity onPress={exitFocusMode} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
        ...
        <Animated.View style={[styles.card, { transform: [{ translateX: topTranslateX }] }]}>
          ...
          <View style={styles.cardOverlay}>
            <View style={styles.cardOverlayRow}>
              ...
              <TouchableOpacity style={styles.infoButton} onPress={() => router.push(`/movie/${topMovie.id}`)}>
        ...
        <Animated.View style={[styles.card, { transform: [{ translateX: bottomTranslateX }] }]}>
          ...
          <View style={styles.cardOverlay}>
            <View style={styles.cardOverlayRow}>
              ...
              <TouchableOpacity style={styles.infoButton} onPress={() => router.push(`/movie/${bottomMovie.id}`)}>
                <Text style={styles.infoButtonText}>ⓘ</Text>
              </TouchableOpacity>
            </View>
          </View>
