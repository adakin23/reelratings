...
- [x] Filter UI upgrade — Genre, Language, Streaming dropdowns replaced chip grids with SearchableDropdown (shows full list on focus, filters as you type). Status filter (Watched/Watchlist/Undiscovered) added to Discover only as multi-select chips.
- [x] Rankings crash fix — optional chaining on `filters.statuses?.length` to handle old AsyncStorage data missing the statuses field
- [x] Genre filter fix — `parseGenres` helper handles three formats: proper objects, JSON strings, plain strings (try/catch fallback). Applied to RankingsScreen, DiscoverScreen, WatchlistScreen.
- [x] Auto-replace removed movies — when user swipes left (do_not_watch), `addReplacementMovie` fires and adds best predicted unseen movie to library (fire-and-forget)
- [x] Near-threshold priority — 70% chance to pick a movie with exactly 4 matchups as movieA in MatchupScreen, ensuring movies reach Rankings faster
- [x] "Not Seen" label — renamed from "Won't Watch" / "don't watch" throughout MatchupScreen
- [x] MovieDetailScreen — removed "Don't Want to Watch" button; status row shows Watched and Watchlist only. "Not Seen" note shown if movie already has do_not_watch status.
- [x] Letterboxd import buttons removed from Rankings and Watchlist screens — import/export deferred to website
- [x] Profile screen — username, email, member since, app version, feedback form (category + message → `feedback` Supabase table), sign out
- [x] Profile icon (👤) in tab bar header — navigates to `/profile` on all four tabs

### Tier 1 — Next to Build (Priority Order)

- [ ] **Onboarding tutorial** — How the app works, swipe directions, ELO explained. Force ~25 matchups (5 movies × 5 each) before main app opens so Rankings isn't empty on day 1.
- [ ] **Rankings: Movies / Actors / Directors tabs** — Three tabs on Rankings screen. Movies tab = current behavior. Actors tab = actors ranked by affinity score. Directors tab = directors ranked by affinity score. *(Deferred — build after movie experience is solid)*

### Tier 2 — High Value

- [ ] Match history + rating trend chart (tap win/loss on movie detail) — already built per session notes; verify it's wired up
- [ ] Data export — CSV download of personal rankings (deferred to website)
...
- [ ] Apple sign-in
- [ ] Website dashboard (Phase 2) — statistics, trends, actor/director leaderboards, Letterboxd import/export
...
| 2026-08-20 | Actors/Directors tabs on Rankings deferred      | Focus on getting movie experience solid first; add actor/director tabs later         |
| 2026-08-20 | Genre/Language/Streaming filters → searchable dropdowns | Show full list on focus, filter as you type; better UX than chip grids        |
| 2026-08-20 | Status filter on Discover only (multi-select)   | Watched/Watchlist/Undiscovered — only meaningful on Discover, not Rankings/Watchlist |
| 2026-08-20 | Genre filter fix via parseGenres helper         | genres column was text[] storing JSON strings; helper handles all three formats with try/catch |
| 2026-08-20 | "Not Seen" replaces "Won't Watch" / "don't watch" | Removes negative connotation; movies can be re-added via MovieDetailScreen |
| 2026-08-20 | do_not_watch button removed from MovieDetailScreen | Status row simplified to Watched + Watchlist only; cleaner UX |
| 2026-08-20 | Letterboxd import/export moved to website       | Too clunky on mobile; will add website link in app once site exists |
| 2026-08-20 | Profile screen with Supabase feedback table     | Simple feedback loop without third-party services; category + message stored for periodic review |
| 2026-08-20 | Profile icon in tab header (not a fifth tab)    | Profile is low-frequency; doesn't need a permanent tab slot |
